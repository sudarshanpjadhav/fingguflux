...
runTest('Prop → Class Resolution (Vue Button Logic)', () => {
    ...
    const devClasses = getClasses({ variant: 'primary', size: 'lg' }, 'dev', null);
    assert(devClasses === 'ff-btn ff-btn-primary ff-btn-lg', `Dev mapping mismatch: ${devClasses}`);
...
runTest('Manifest Verification', () => {
    // This simulates checking exported constants
    const __ffClasses_Button = ['ff-btn', 'ff-btn-primary', 'ff-btn-secondary'];
    assert(__ffClasses_Button.includes('ff-btn-primary'), 'Manifest missing classes');
});

console.log("\n🚀 All Vue Logic Tests Passed!");
